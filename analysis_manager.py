...
class AnalysisManager:
    ...
    def extract_keywords_from_script(self, script, ai_available, model):
        """AI 또는 로컬 방식으로 대본에서 키워드 5개 추출"""
        extracted_keywords = []
        if ai_available and len(script) > 50:
            try:
    ...
    def analyze_speech_style(self, transcript, mapped_mode):
        """(텍스트 기반) 종결어미 패턴을 분석하여 어조 피드백 생성"""
    ...
    def generate_ai_feedback(self, client, script, target_type, delivery_metrics, style_feedback, energy_feedback):
        """(OpenAI) 모든 분석 결과를 종합하여 LLM에게 최종 리포트 생성 요청"""
        rubric = self.COACHING_CONFIG["rubrics"][target_type]
        print(f"🤖 [{rubric['type_name']}] 기준으로 심층 코칭 리포트 작성 중...")
        ...
        - 어조 피드백 (텍스트 기반): "{style_feedback}"
        - 에너지 피드백 (오디오 기반): "{energy_feedback}"
        ...
        **🛠️ 개선 솔루션**
        1. (내용/구조 측면 1가지)
        2. (전달력/어조/에너지 측면 1가지 - *자동 분석 데이터를 근거로 제시*)
        **💡 총평** (따뜻한 격려)
        """
